...
void Compression::tea_decrypt(uint8_t* dest,
	const uint8_t* src,
	uint32_t size,
	const uint32_t key[4])
{
	...
	// tea already does its own padding adjustment.
...
bool Compression::decrypt_and_decompress(const uint8_t* encrypted, uint32_t encrypted_size, const uint32_t key[4], std::vector<uint8_t>& output)
{
	...
	if (header->encrypt_size != 0)
	{
		...
		const uint8_t* encryptedRegion = encrypted + sizeof(Header);

		decrypted.resize(header->encrypt_size);
		tea_decrypt(decrypted.data(), encryptedRegion, header->encrypt_size, key);
